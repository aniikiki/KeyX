...
Nintendo Switch 按键助手，支持连发、按键重新分配、按键宏和触摸映射。且拥有全局或游戏独立配置，根据记忆自动启动功能。
...
- 支持多个按键同时连发
- 连发频率可在每秒 1～10 次之间调整
- 可选一个始终透传的连发功能键：未设置时保持直接连发；设置后需组合触发按住或切换连发
- 每个按键可独立选择按住连发或按下切换连发；切换连发通过单独的停止键关闭
- 停止键和功能键不能同时设为连发键；停止键按下后停止所有切换连发，不影响仍被按住的按住连发
- 连发时支持非连发键正常使用
- 可设置按下和松开时长
- 全局配置和游戏独立配置
- 自动记忆开关状态
- KeyX 的 Tesla 前端打开时会暂停连发、宏、触摸映射和按键映射

### 触摸映射

- 屏幕左半区域和右半区域可分别映射为一个按键，并可同时触发
- 手指落下时确定区域，滑过屏幕中线不会改变本次触摸的目标按键
- 映射出的按键会复用该按键现有的普通、按住连发、切换连发、停止键和功能键逻辑
- 点击输出至少保持 50ms，避免过短触摸丢失
- 支持全局配置和游戏独立配置，并自动记忆开关状态
- 不拦截游戏原始触摸事件，主要用于本身不支持触摸操作的游戏
...
Nintendo Switch button assistant supporting turbo, key remapping, macro recording, and touch mapping. Features global or per-game configuration with auto-start memory.
...
- Each button can independently use hold-to-turbo or press-to-toggle turbo; toggle turbo uses a separate stop key
- The stop key cannot also be a turbo key; it stops all toggle turbo without interrupting held hold-to-turbo buttons
- Non-turbo buttons work normally during turbo
- Customizable press and release duration
- Global and per-game configuration
- Auto-remembers on/off state
- Turbo, macros, touch mapping, and button mapping pause while the KeyX Tesla frontend is open

### Touch Mapping

- Map the left and right halves of the screen to separate buttons and use both at once
- The initial touch position locks the region even if the finger crosses the center line
- Synthetic buttons reuse their normal, hold-turbo, toggle-turbo, and stop-key behavior
- Each tap is held for at least 50ms so very short touches are not lost
- Supports global and per-game configurations with remembered on/off state
- Original touch events are not intercepted; this is intended mainly for games without touch controls
